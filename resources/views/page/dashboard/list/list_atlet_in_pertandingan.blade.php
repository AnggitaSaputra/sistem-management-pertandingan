...
<script>
    ...
    function populateTable(response) {
        ...
        response.forEach(function(data, index) {
            ...
            row.append($('<td>').addClass('px-6 py-4').text(data.atlet.nama));
    ...
    function editAtlet(id) {
        $.ajax({
            url: `http://127.0.0.1:8000/jadwal/pertandingan/list/tim/atlet/update/${id}`,
            type: 'GET',
            success: function(response) {
                $('#id').val(response.id);
    ...
    function deleteAtlet(id) {
        if (confirm('Apakah anda yakin ingin menghapus atlet ini?')) {
            $.ajax({
                url: `http://127.0.0.1:8000/jadwal/pertandingan/list/tim/atlet/delete/${id}`,
                type: 'GET',
                success: function(response) {
                    alert(response);
                    fetchData();
